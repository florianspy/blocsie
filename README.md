...
The depth camera (DepthAngleMaterialChannel.cs), which publishes the material and angle of hit information, is available here (published topics set via cameratopic, caminfotopic). This camera publishes the distances in float.
     * BoundBox publishes BoundingBox data. If an object for which a label was assigned for. To assign a label left click on an Gameobject. Then select "Add Component" and choose Labeling. Next select "Use Automatic Labeling" and under Labeling Scheme select "use asset name". Afterwards, press "Add to Label Config..." . In the next window push the "Add Label" Button next to "IdLabelConfig".
