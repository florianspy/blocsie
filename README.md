...
* The camera sensor(s) can be found below the Gameobject "d415". It allows one to set the camera resolution (via Script), the distortion parameters (via Script), frame rate (via Script), the name of the topics (published topics set via cameratopic, caminfotopic via the Script), the field of view (via the Camera itself), and near and far clipping (via the Camera itself).
