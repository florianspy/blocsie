...
int main()
{
	...
	//reader_grey_intensity.set_target_file_path(path_to_res+"SICK_grey_interpolate.txt");
	//reader_grey_intensity.WriteFile(sick.sigma_with_deg_dis());
	//example code to interpolate data when measured at all distances / degrees for sick with offset approach as in this case the intensity values are not used you can uses those of grey_intensity
